...
trait Route[M[_]]:
   ...
   *  * If the above parsing is successfull - return the value generated by the parser.
   *  * Otherwise if the header `header.name` was not present, return the `defaultValue`
  ...
  /** Routes the request based on the header value. */
  def header[T](name: String, fn: PartialFunction[Option[String], M[T]]): M[T]
...
object Route:
  ...
  /**
   * Routes the request based on the header value. If multiple headers with the same
   * name are present, they are combined into the comma-separated string. However it
   * would be unwise to use this method with the headers that may have many values as
   * in many cases order of the values could not be guaranteed.
   *
   * The most common use-case is request parsing:
   * ```
   * def parseContent(): M[Content] =
   *   Route.header("Content-Type") {
   *     case Some("application/json") => parseJson()
   *     case Some("text/xml") => parseXml()
   *   }
   * ```
   */
  inline def header[M[_], T](
        headerName: String,
      )(
        fn: PartialFunction[Option[String], M[T]],
      )(using
        rt: Route[M],
      ): M[T] =
    rt.header(headerName, fn)
