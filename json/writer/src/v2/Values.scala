...
object Values {
   ...
   */
  def createCompactWriter[M[_]: Monad, T, S: Writer.In[M]](classifier: ValueClassifier[T]): JsonWriter[M, S, T] =
    new CompactWriter(classifier)


  /** Writes the value in the most compact form possible. */
  def writeCompact[M[_]: Monad, T, B >: T, S: Writer.In[M]](
        value: T,
        stream: S,
      )(implicit
   ...
   * @return reusable writer that is able to produce compact JSON representations.
   */
  def createPrettyWriter[M[_]: Monad, T, S: Writer.In[M]](
  ...
  def writePretty[M[_]: Monad, T, B >: T, S: Writer.In[M]](
        format: PrettyPrintOptions,
        value: T,
        stream: S,
      )(implicit
