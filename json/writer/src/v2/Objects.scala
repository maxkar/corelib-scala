...
object Objects {
   ...
   */
  final class Layout[M[_]: Monad, S : Writer.In[M]] private[Objects](
  ...
  private final class WriterImpl[M[_]: Monad, S : Writer.In[M], T](
        layout: Layout[M, S],
  ...
  def writeObjectStart[M[_], S: Writer.In[M]](stream: S): M[Unit] =
    stream.write(OBJECT_START)


  /** Writes object end sequence (epilogue). */
  def writeObjectEnd[M[_], S: Writer.In[M]](stream: S): M[Unit] =
    stream.write(OBJECT_END)


  /** Writes key-value separator. */
  def writeKeyValueSeparator[M[_], S: Writer.In[M]](stream: S): M[Unit] =
    stream.write(KEY_VALUE_SEPARATOR)


  /** Writes element (key-value-pair) separator. */
  def writeElementSeparator[M[_], S: Writer.In[M]](stream: S): M[Unit] =
   ...
   */
  def beginObject[M[_]: Monad, S : Writer.In[M]](
        whitespaces: Whitespaces[M, S],
        stream: S
      ): M[Layout[M, S]] =
    for
   ...
   */
  def newWriter[M[_]: Monad, S : Writer.In[M], T](
        whitespaces: Whitespaces[M, S],
        writeElement: (T, S) => M[Unit],
        stream: S,
      ): M[Writer[M, T]] =
  ...
  def writeAll[M[_]: Monad, S : Writer.In[M], T](
        whitespaces: Whitespaces[M, S],
        writeElement: (T, S) => M[Unit],
