...
object Arrays {
   ...
   */
  final class Layout[M[_]: Monad, S : Writer.In[M]] private[Arrays](
  ...
  def writeArrayStart[M[_], S: Writer.In[M]](stream: S): M[Unit] =
    stream.write(ARRAY_START)


  /** Writes array end (epilogue). */
  def writeArrayEnd[M[_], S: Writer.In[M]](stream: S): M[Unit] =
    stream.write(ARRAY_END)


  /** Writes array separator. */
  def writeArraySeparator[M[_], S: Writer.In[M]](stream: S): M[Unit] =
   ...
   */
  def beginArray[M[_]: Monad, S : Writer.In[M]](
        whitespaces: Whitespaces[M, S],
        stream: S
      ): M[Layout[M, S]] =
    for
